...
**Objective:** create a discoverable, installable but functionally disabled OC3 payment extension.

**Expected files/components:** package skeleton; admin controller/model/languages/template; bootstrap/constants; install/uninstall/OCMOD shell; packaging script.

**Tasks:** implement classic OC3 names/routes; settings permission/validation; idempotent install; module-scoped OCMOD/install cleanup; health/readiness report; deterministic package.

**Dependencies:** Phase 0.

**Tests:** OC3 discovery, PHP-floor lint, install twice, permission denial, package layout, uninstall preserves data tables.

**Runtime/manual verification:** install package, permissions, settings per store, modification records/generated files and logs.

**Acceptance criteria:** extension installs/enables/disables without storefront output or warnings and reports missing prerequisites safely.
...
**Objective:** expose UniCredit correctly in native OC3 checkout and render scheme selection, stopping before CP creation.

**Expected files/components:** catalog payment model/controller/template/languages; eligibility/order adapters; submission issuer; minimal checkout JS/CSS.

**Tasks:** implement `getMethod`; derive totals; reuse native `session.order_id`; validate cart/order parity; issue token and validate selection; define visible pending policy; no remote side effect.

**Dependencies:** Phases 3–4.

**Tests:** eligibility combinations, guest/logged/no-shipping, order-total authority, posted identity ignored, duplicate token, no second `addOrder()`.

**Runtime/manual verification:** native checkout on default theme through confirm preparation; inspect order/session/status and asset loading.
