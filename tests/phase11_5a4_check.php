...
if (!defined('DIR_STORAGE')) {
    mtuc_test_define_dir_storage('mtuc-phase115a4');
}
...
/**
 * Mirror CP ShopModuleSmartUcfDebugFetcher sanitize (AUD-028-F03 envelope).
 *
...
{
    return mtuc_cp_smartucf_debug_sanitize($storeResponse);
}
...
$toApi = mtuc115a4_invoke_debug($stackTo, $orderTo);
$toCp = mtuc115a4_cp_sanitize($toApi['payload']);
$toCpLog = isset($toCp['data']['log']) ? $toCp['data']['log'] : null;
mtuc115a4_assert(
    is_array($toCpLog)
        && $toCpLog['response'] === null
        && is_string($toCpLog['transport_error'])
        && $toCpLog['transport_error'] !== ''
        && !isset($toCpLog['response']['sucfOnlineSessionID']),
    'AUD-028-F03 timeout: CP preserves transport_error, no synthetic session'
);
