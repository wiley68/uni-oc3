...
/**
 * DB wrapper: before production bank-status UPDATE, run a concurrent writer (B).
 */
final class MtUniCreditAud015RaceDb
{
    /** @var Phase2MemoryDb */
    private $inner;

    /** @var callable|null */
    public $beforeBankStatusUpdate;
    ...
    {
        if (
            is_callable($this->beforeBankStatusUpdate)
            && stripos(trim($sql), 'UPDATE') === 0
            && strpos($sql, 'order_bank_status') !== false
        ) {
            $cb = $this->beforeBankStatusUpdate;
            $this->beforeBankStatusUpdate = null;
            call_user_func($cb);
        }
...
/**
 * Execute AUD-015 focused checks (kept out of {main} for Intelephense).
 *
 * @param string $lib
 * @return void
 */
function mtucAud015_run($lib)
{
    $failures = array();
    $passes = 0;
    ...
    mtucAud015_assert(
        strpos($repoSrc, 'allowedFromStatusIds') !== false
            && strpos($repoSrc, '$currentId') !== false
            && strpos($repoSrc, 'F01-R1') !== false,
        'F01-R1: CAS includes exact observed current predecessor for numeric ALLOW'
    );
    mtucAud015_assert(
    ...
    // ---------------------------------------------------------------------------
    // F01-R1 — numeric current status must CAS-apply when policy ALLOW
    // ---------------------------------------------------------------------------
    $numericFrom = '42';
    $numericCases = array(
        array('42→P1', MtUniCreditBankStatus::SENT_PROCESS1, $local),
        array('42→P2', MtUniCreditBankStatus::SENT_PROCESS2, $local),
        array('42→FS', MtUniCreditBankStatus::SEND_FAILED_SMARTUCF, $local),
        array('42→FC', MtUniCreditBankStatus::SEND_FAILED_CP, $local),
        array('42→cp_sent', MtUniCreditBankStatus::CP_SENT, $inbound),
        array('42→smartucf_sent', MtUniCreditBankStatus::SMARTUCF_SENT, $inbound),
        array('42→43', '43', $inbound),
        array('42→bank_send_failed', MtUniCreditBankStatus::SEND_FAILED, $inbound),
    );
    $oidNum = 15150;
    foreach ($numericCases as $i => $case) {
        $memory = new Phase2MemoryDb();
        $repo = mtucAud015_repo($memory, $storeId, $oidNum + $i);
        $seed = mtucAud015_write($repo, $storeId, $oidNum + $i, $numericFrom, 'External 42', $inbound);
        mtucAud015_assert(
            $seed !== null && !empty($seed['applied']) && (string) $seed['status_id'] === $numericFrom,
            'F01-R1 seed numeric for ' . $case[0]
        );
        mtucAud015_assert(
            MtUniCreditBankStatusTransitionPolicy::decide($numericFrom, $case[1], $case[2])
                === MtUniCreditBankStatusTransitionPolicy::DECISION_ALLOW,
            'F01-R1 policy ALLOW ' . $case[0]
        );
        $adv = mtucAud015_write(
            $repo,
            $storeId,
            $oidNum + $i,
            $case[1],
            MtUniCreditBankStatus::resolveLabel($case[1], 'numeric-advance'),
            $case[2]
        );
        $row = $repo->findByOrderId($storeId, $oidNum + $i);
        mtucAud015_assert(
            $adv !== null
                && !empty($adv['applied'])
                && $row !== null
    ...
    // Numeric same-status NO-OP
    $memory = new Phase2MemoryDb();
    $repo = mtucAud015_repo($memory, $storeId, 15170);
    mtucAud015_write($repo, $storeId, 15170, $numericFrom, 'External 42', $inbound);
    $noopNum = mtucAud015_write($repo, $storeId, 15170, $numericFrom, 'External 42 again', $inbound);
    $row = $repo->findByOrderId($storeId, 15170);
    mtucAud015_assert(
        $noopNum !== null
            && empty($noopNum['applied'])
            && $row !== null
    ...
    // Stale numeric CAS race: A reads 42, B writes P1, A attempts 42→cp_sent → CAS miss, final P1
    $memory = new Phase2MemoryDb();
    $orderRace = 15190;
    $memory->seedOrder($orderRace, $storeId, MtUniCreditConstants::EXTENSION_CODE);
    $raceDb = new MtUniCreditAud015RaceDb($memory);
    $repoA = new MtUniCreditOrderBankStatusRepository(new MtUniCreditDbAdapter($raceDb, 'oc_'));
    $repoB = new MtUniCreditOrderBankStatusRepository(new MtUniCreditDbAdapter($memory, 'oc_'));
    $seedRace = mtucAud015_write($repoB, $storeId, $orderRace, $numericFrom, 'External 42', $inbound);
    mtucAud015_assert(
        $seedRace !== null && !empty($seedRace['applied']),
        'F01-R1 race seed 42'
    );
    $raceDb->beforeBankStatusUpdate = function () use ($repoB, $storeId, $orderRace, $local) {
        mtucAud015_write(
            $repoB,
            $storeId,
            $orderRace,
    ...
    $row = $repoB->findByOrderId($storeId, $orderRace);
    mtucAud015_assert(
        $staleA !== null
            && empty($staleA['applied'])
            && $row !== null
            && (string) $row['status_id'] === MtUniCreditBankStatus::SENT_PROCESS1,
        'F01-R1 stale numeric CAS: A rejected, final P1'
    );
